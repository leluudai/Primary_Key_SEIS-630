--------------------------- START OF SECTION B-----------------------------------------------
/* Part C of Exhibit B (Assumption with single project) */
...
-- Note:  The last row is the Total Hours the sum of all the above rows combined.
...
------Exhibit D, Part D
-- Assumption here is return a single employee
...
---------------- This is create view section for Exhibit D - Part E --------------------------
...
WHERE (P.PROJECTNUMBER= 'IL-SJB-335-005') and e.SSN = '392224484' and d.Total_Hours <= 40 and (TO_CHAR(d.STARTTIME, 'DD-MON-YYYY') BETWEEN '04-MAR-2018' AND '10-MAR-2018')
GROUP BY s.SKILLCODE, P.PROJECTID;
--------------------- End of create view section for Exhibit D - Part E --------------------------
...
-- Assumption for a single employee
SELECT SSN, LASTNAME, FIRSTNAME, MI, STREET, CITY, STATE, ZIP, PHONE, TO_CHAR(DOB, 'DD MONTH YYYY') as Date_Of_Birth, GENDER, MARITALSTATUS, EEOTYPE
FROM EMPLOYEE
WHERE EmployeeID = 1;
--------------------------- END OF SECTION E-----------------------------------------------
...
-- Assumption for a single employee with a 2 week pay period.
SELECT PROJECTNAME, SKILLCLASSIFICATION AS Skill, BASICHOURLYRATE, FRINGE, BASICHOURLYRATE + FRINGE AS TOTAL, SUM((ENDTIME-STARTTIME)*24) AS HOURS, (BASICHOURLYRATE + FRINGE) * SUM((ENDTIME-STARTTIME)*24) AS GROSSPAY
FROM EMPLOYEE
