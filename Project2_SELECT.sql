...
SELECT NMFH.SKILLCODE, NMFH.SKILLCLASSIFICATION, MINORITY_MALE, MINORITY_FEMALE, NON_MINORITY_FEMALE
FROM NON_MINORITY_FEMALE_HOURS NMFH
JOIN NON_MINORITY_MALE_HOURS NMMH on NMFH.SKILLCODE = NMMH.SKILLCODE
...
CREATE VIEW DETAIL_WORK_HOUR_MAR10 AS Select TIMECARDID, SKILLID, EMPLOYEEID, PROJECTID, STARTTIME, ENDTIME, TO_NUMBER(TO_CHAR(STARTTIME,'HH24'))
...
SELECT e.SSN, s.SKILLCODE, s.BASICHOURLYRATE AS RATE, s.FRINGE, (s.BASICHOURLYRATE*1.5 + s.FRINGE) AS TOTAL, SUM(t.TOTALTIME) as Hours, ((s.BASICHOURLYRATE*1.5 + s.FRINGE) * t.TOTALTIME) AS GROSS
FROM EMPLOYEE e
JOIN DETAIL_WORK_HOUR_MAR10 d ON e.EMPLOYEEID = d.EMPLOYEEID
JOIN TIME_CARD t ON t.TIMECARDID = d.TIMECARDID
JOIN SKILL s ON s.SKILLID = d.SKILLID
JOIN PROJECT P on t.PROJECTID = P.PROJECTID
WHERE (P.PROJECTNUMBER= 'IL-SJB-335-005') and d.Total_Hours > 40 and (TO_CHAR(d.STARTTIME, 'DD-MON-YYYY') BETWEEN '04-MAR-2018' AND '10-MAR-2018')
GROUP BY e.SSN, s.SKILLCODE, s.BASICHOURLYRATE, s.FRINGE, t.TOTALTIME;

------Exhibit D, Part F
SELECT s.SKILLCODE, SUM(t.TOTALTIME) AS OVERTIME
FROM DETAIL_WORK_HOUR_MAR10 d
JOIN TIME_CARD t ON t.TIMECARDID = d.TIMECARDID
JOIN SKILL s ON s.SKILLID = d.SKILLID
JOIN PROJECT P on t.PROJECTID = P.PROJECTID
WHERE (P.PROJECTNUMBER= 'IL-SJB-335-005') and d.Total_Hours > 40 and (TO_CHAR(d.STARTTIME, 'DD-MON-YYYY') BETWEEN '04-MAR-2018' AND '10-MAR-2018')
...
SELECT s.SKILLCODE, SUM(t.TOTALTIME) AS REGULAR
FROM DETAIL_WORK_HOUR_MAR10 d
JOIN TIME_CARD t ON t.TIMECARDID = d.TIMECARDID
JOIN SKILL s ON s.SKILLID = d.SKILLID
JOIN PROJECT P on t.PROJECTID = P.PROJECTID
...
SELECT Total_Hours FROM DETAIL_WORK_HOUR WHERE EMPLOYEEID = 1 AND Total_Hours = 40;
